...
export class AddressFormatRepository {
  private formats: Map<string, AddressFormat> = new Map();
  ...
  private loadHardcodedData(): void {
    ...
    for (const [countryCode, data] of Object.entries(formatData)) {
      const format = new AddressFormat(
        countryCode,
        data.format,
        data.requiredFields,
        data.upperFields,
        data.administrativeAreaType,
        data.localityType,
        data.dependentLocalityType,
        data.postalCodeType,
        data.postalCodePattern,
        data.postalCodePrefix
      );
   ...
   */
  async get(countryCode: string): Promise<AddressFormat | null> {
   ...
   */
  async getAll(): Promise<AddressFormat[]> {
    await this.initialize();
